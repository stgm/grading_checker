...
module GradingChecker
    ...
    class FileManager
        ...
        def merge_configs(grades1, grades2)
            first_without_grades = grades1.reject { |k, _| ['templates', 'grades', 'calculation'].include?(k) }
            second_without_grades = grades2.reject { |k, _| ['templates', 'grades', 'calculation'].include?(k) }
    ...
    class GradingYAMLValidator
        VALID_GRADE_TYPES = ['pass', 'float', 'integer', 'points'].freeze
        VALID_SUBGRADE_TYPES = ['integer', 'pass', 'boolean', 'float'].freeze
